...
import { role, farm, userAccount, batchOperation, saleSettlement } from "@/db/schema";
...
describe("sale_settlement table", () => {
  it("stores a liquidación linked to a batch operation, with its PDF attached", async () => {
    ...
    const [stored] = await testDb.select().from(saleSettlement).where(eq(saleSettlement.id, created.id));
