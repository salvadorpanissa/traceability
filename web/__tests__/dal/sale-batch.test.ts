...
async function seedSaleEvent(
  ...
  const [saleEvent] = await testDb
    .insert(event)
    .values({ eventType: "sale", eventDate, animalId: createdAnimal.id, farmId, batchOperationId: batchId, createdBy: userId })
    .returning();
  await testDb.insert(eventSale).values({ eventId: saleEvent.id, guideNumber, buyer, price, weightKg });
...
describe("findSaleBatchByGuideNumber", () => {
  ...
  it("groups every animal sold under the same guide number into one match", async () => {
    ...
    const [batch] = await testDb
      ...
      .values({ eventType: "sale", farmId: seededFarm.id, animalCount: 2, createdBy: user.id })
      .returning();
