...
async function seedRegistration() {
  ...
  const [registration] = await testDb
    ...
    .returning();
  return { registration, user, seededFarmGroup };
}
...
describe("importOwnTags", () => {
  it("registers bare tags, ignoring blank and invalid values, without creating any animal", async () => {
    ...
    const animals = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "100"));
  ...
  it("creates the animal right away when sex, category, or birth date is given — even with no paddock", async () => {
    ...
    const [history] = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "300"));
    const [createdAnimal] = await testDb
      .select()
      .from(animal)
      .where(eq(animal.id, history.animalId));
  ...
  it("stores a month/year-only birth date (herd records rarely track the exact day)", async () => {
    ...
    const [history] = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "304"));
    const [createdAnimal] = await testDb
      .select()
      .from(animal)
      .where(eq(animal.id, history.animalId));
  ...
  it("leaves sex, category, and birth date null and does not create an animal when unrecognized or unmapped", async () => {
    ...
    const animals = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "301"));
    expect(animals).toHaveLength(0);
  });
  ...
  it("does not create an animal for a bare tag with no sex/category/birth date/paddock", async () => {
    ...
    const animals = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "400"));
    expect(animals).toHaveLength(0);
  });
  ...
  it("creates and locates an animal in the given paddock", async () => {
    ...
    const [history] = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "500"));
  ...
  it("does NOT auto-create a missing paddock — leaves the animal establishment-located without a paddock instead", async () => {
    ...
    const [createdPaddock] = await testDb
      .select()
      .from(paddock)
      .where(eq(paddock.name, "Potrero Nuevo"));
  ...
  it("creates the animal on re-upload for a tag registered earlier as bare, once a paddock is given", async () => {
    ...
    const [history] = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "502"));
    expect(history).toBeDefined();
  ...
  it("recategorizes an already-existing animal on re-upload when the category column is mapped later", async () => {
    ...
    const [history] = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "600"));
    const beforeState = await testDb.execute<{
      current_category_id: string | null;
    }>(
    ...
    const afterState = await testDb.execute<{
      current_category_id: string | null;
      current_paddock_id: string | null;
    }>(
  ...
  it("backfills sex and birth date on an already-existing animal that didn't have them", async () => {
    ...
    const [history] = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "601"));
    const [beforeAnimal] = await testDb
      .select()
      .from(animal)
      .where(eq(animal.id, history.animalId));
    ...
    const [afterAnimal] = await testDb
      .select()
      .from(animal)
      .where(eq(animal.id, history.animalId));
  ...
  it("does not recategorize an animal that already has a category, even if the upload brings a different one", async () => {
    ...
    const [history] = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "602"));
  ...
  it("sets breed on a newly created animal and secondaryTag on its tag history row", async () => {
    ...
    const [tagRow] = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "700"));
  ...
  it("gap-fills breed on an existing animal that has none yet", async () => {
    ...
    const [tagRow] = await testDb
      .select()
      .from(animalTagHistory)
      .where(eq(animalTagHistory.tag, "701"));
