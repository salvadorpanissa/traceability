...
describe("PaddockCatalogForm", () => {
  it("lists paddocks with their establishment, adds a new one, and edits an existing one", async () => {
    vi.mocked(createPaddockAction).mockResolvedValue({
      ok: true,
      entry: { id: "pad-2", name: "Potrero 2", establishmentId: "establishment-1" },
    });
    vi.mocked(updatePaddockAction).mockResolvedValue({
      ok: true,
      entry: { id: "pad-1", name: "Potrero 1 (bajo)", establishmentId: "establishment-1" },
    });

    render(
      <PaddockCatalogForm
        paddocks={[{ id: "pad-1", name: "Potrero 1", establishmentId: "establishment-1" }]}
        establishments={[{ id: "establishment-1", name: "Campo Norte" }]}
      />
    );
    ...
    expect(createPaddockAction).toHaveBeenCalledWith({ establishmentId: "establishment-1", name: "Potrero 2" });
  ...
  it("preselects the establishment and disables the add button when the user has none", async () => {
    render(<PaddockCatalogForm paddocks={[]} establishments={[]} />);
  ...
  it("shows an inline error when the name is a duplicate within the establishment", async () => {
    vi.mocked(createPaddockAction).mockResolvedValue({
      ok: false,
      error: "Ya existe un potrero con ese nombre en ese campo",
    });

    render(<PaddockCatalogForm paddocks={[]} establishments={[{ id: "establishment-1", name: "Campo Norte" }]} />);
