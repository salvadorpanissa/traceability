...
describe("confirmDeathEvent", () => {
  it("writes a death event with cause and flips animal_current_state.status to dead", async () => {
    ...
    const [deathEvent] = await testDb
      ...
      .from(event)
      .where(
        sql`${event.animalId} = ${createdAnimal.id} and ${event.eventType} = 'death'`,
      );
    expect(deathEvent).toBeDefined();
    const [deathRow] = await testDb
      .select()
      .from(eventDeath)
      .where(sql`${eventDeath.eventId} = ${deathEvent.id}`);
  ...
  it("accepts a null cause", async () => {
    ...
    const [deathEvent] = await testDb
      .select()
      .from(event)
      .where(sql`${event.eventType} = 'death'`);
    const [deathRow] = await testDb
      .select()
      .from(eventDeath)
      .where(sql`${eventDeath.eventId} = ${deathEvent.id}`);
  ...
  it("rejects an unknown tag", async () => {
    ...
    await expect(
      ...
      "No se encontró esa caravana o no tenés acceso a su campo",
    );
  });
