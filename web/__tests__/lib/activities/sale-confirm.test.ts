...
describe("confirmSaleBatch", () => {
  it("creates a sale event with guideNumber/buyer/price/weightKg for a new animal, plus its placement traslado", async () => {
    ...
    const [tagRow] = await testDb
      ...
      .where(eq(animalTagHistory.tag, "AR000000000200"));
  ...
  it("does not create a placement traslado for an existing animal", async () => {
    ...
    await testDb
      ...
      .values({ animalId: createdAnimal.id, tag: "AR000000000201" });
    const rows: ResolvedRow[] = [
      {
        tag: "AR000000000201",
        eventDate: "2026-02-01",
  ...
  it("gap-fills breed and secondaryTag on an existing animal that has neither yet", async () => {
    ...
    await testDb
      .insert(animalTagHistory)
      .values({ animalId: createdAnimal.id, tag: "AR000000000209" });
    const rows: ResolvedRow[] = [
      {
        tag: "AR000000000209",
        eventDate: "2026-02-01",
  ...
  it("stores null buyer/price/weightKg when omitted", async () => {
    ...
    await testDb
      .insert(animalTagHistory)
      .values({ animalId: createdAnimal.id, tag: "AR000000000202" });
    const rows: ResolvedRow[] = [
      {
        tag: "AR000000000202",
        eventDate: "2026-02-01",
    ...
    const [saleRow] = await testDb
      .select()
      .from(eventSale)
      .where(eq(eventSale.eventId, animalEvents[0].id));
  ...
  it("rejects the batch when an existing row's animal is currently at another establishment", async () => {
    ...
    await testDb
      .insert(animalTagHistory)
      .values({ animalId: createdAnimal.id, tag: "AR000000000208" });
    const rows: ResolvedRow[] = [
      {
        tag: "AR000000000208",
        eventDate: "2026-02-01",
  ...
  it("stores the guide PDF on the batch operation", async () => {
    ...
    await testDb
      .insert(animalTagHistory)
      .values({ animalId: createdAnimal.id, tag: "AR000000000207" });
    const rows: ResolvedRow[] = [
      {
        tag: "AR000000000207",
        eventDate: "2026-02-01",
    ...
    const [batch] = await testDb
      .select()
      .from(batchOperation)
      .where(eq(batchOperation.eventType, "sale"));
