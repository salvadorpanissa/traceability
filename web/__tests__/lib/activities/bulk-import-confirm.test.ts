...
function validRow(farmId: string, overrides: Partial<Extract<ResolvedImportRow, { status: "valid" }>> = {}) {
  return {
...
describe("confirmImportChunk", () => {
  it("creates the animal, its tag history with breed/secondaryTag, and the three initial events", async () => {
    ...
    const [tagRow] = await testDb.select().from(animalTagHistory).where(eq(animalTagHistory.tag, "858000048233520"));
    ...
    expect(events.map((e) => e.eventType).sort()).toEqual(["recategorize", "retag", "transfer"]);
  });
  ...
  it("creates a new owner by name and links it to the animal", async () => {
    ...
    await confirmImportChunk({ userId: admin.id, rows: [validRow(seededFarm.id)] });

    const [createdOwner] = await testDb.select().from(owner).where(eq(owner.name, "SASG"));
  ...
  it("reuses an existing owner instead of creating a duplicate", async () => {
    const admin = await seedAdmin();
    const [seededFarm] = await testDb.insert(farm).values({ name: "San Antonio" }).returning();
    const [existingOwner] = await testDb.insert(owner).values({ name: "SASG" }).returning();

    await confirmImportChunk({ userId: admin.id, rows: [validRow(seededFarm.id)] });

    const owners = await testDb.select().from(owner).where(eq(owner.name, "SASG"));
  ...
  it("creates a new paddock scoped to the row's farm, and a new category, reusing them across rows in the same chunk", async () => {
    ...
    const paddocks = await testDb.select().from(paddock).where(eq(paddock.name, "Arerunguá"));
    expect(paddocks).toHaveLength(1);
    expect(paddocks[0].farmId).toBe(seededFarm.id);

    const categories = await testDb.select().from(category).where(eq(category.name, "Vaca de cría"));
    ...
    const [transfer] = await testDb.select().from(eventTransfer).where(eq(eventTransfer.destinationFarmId, seededFarm.id));
  ...
  it("does not create a recategorize event when the row has no category name", async () => {
    ...
    await confirmImportChunk({ userId: admin.id, rows: [validRow(seededFarm.id, { categoryName: null })] });
  ...
  it("groups rows from different farms into separate batch_operation rows", async () => {
    const admin = await seedAdmin();
    const [farmA] = await testDb.insert(farm).values({ name: "San Antonio" }).returning();
    const [farmB] = await testDb.insert(farm).values({ name: "Cuatro Cerros" }).returning();

    await confirmImportChunk({
      userId: admin.id,
      rows: [validRow(farmA.id, { tag: "TAG1" }), validRow(farmB.id, { tag: "TAG2" })],
    });
  ...
  it("leaves animal_current_state reflecting the imported tag, category, and paddock", async () => {
    ...
    const [tagRow] = await testDb.select().from(animalTagHistory).where(eq(animalTagHistory.tag, "858000048233520"));
    const stateResult = await testDb.execute<{ current_tag: string | null; current_farm_id: string | null }>(
