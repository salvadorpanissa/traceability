...
async function seedFarmUserRole(establishmentName = "Campo Norte") {
  ...
  const [user] = await testDb
    ...
    .returning();
  return { seededFarm, seededFarmGroup, user };
}
...
describe("resolveBatchRows", () => {
  it("resolves an existing, alive animal with its current location", async () => {
    const { seededFarm, createdAnimal } =
      await seedExistingAnimal("AR000000000001");
  ...
  it("errors a sold or dead animal", async () => {
    const { seededFarm } = await seedExistingAnimal("AR000000000002", {
      sold: true,
    });
    const rows: MappedRow[] = [
      {
  ...
  it("infers the owner from the tag's DICOSE registration, ignoring the Excel owner column", async () => {
    const { seededFarm } = await seedFarmUserRole();
    const registeredOwner = await seedOwnTag(
      "AR000000000032",
      seededFarm.id,
      "AIP",
    );
  ...
  it("errors a row whose secondaryTag already belongs to a different animal", async () => {
    const { seededFarm, createdAnimal: otherAnimal } =
      await seedExistingAnimal("AR000000000094");
    await testDb
      .update(animalTagHistory)
  ...
  it("does not error when a row's secondaryTag matches its own already-resolved animal", async () => {
    const { seededFarm, createdAnimal } =
      await seedExistingAnimal("AR000000000096");
    await testDb
      .update(animalTagHistory)
