...
const { runNaturalLanguageQuery } =
  await import("@/app/(protected)/dashboard/query-actions");
const { auth } = await import("@/auth");

beforeEach(async () => {
  await resetTestDb();
  process.env.NL_QUERY_TEST_SQL_OVERRIDE =
    "SELECT status, count(*) as total FROM my_animal_state GROUP BY status";
...
describe("runNaturalLanguageQuery", () => {
  it("returns only the manager's farm data even when the generated SQL has no farm filter", async () => {
    ...
    const result = await runNaturalLanguageQuery(
      "¿cuántos animales por estado?",
    );
