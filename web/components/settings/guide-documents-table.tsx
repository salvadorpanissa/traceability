...
export function GuideDocumentsTable({ rows, locale }: { rows: GuideDocumentSummary[]; locale: Locale }) {
  ...
  const columns: DataTableColumn<GuideDocumentSummary>[] = [
    ...
    },
    {
      key: "originFarm",
      header: "Campo origen",
      render: (row) => row.originFarmName ?? "—",
      sortValue: (row) => row.originFarmName,
      searchValue: (row) => row.originFarmName ?? "",
    },
    {
      key: "destinationFarm",
      header: "Campo destino",
      render: (row) => row.destinationFarmName,
      sortValue: (row) => row.destinationFarmName,
      searchValue: (row) => row.destinationFarmName,
    },
    {
      key: "animalCount",
      header: "Animales",
